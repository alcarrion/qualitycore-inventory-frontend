...
export const ERRORS = {
  ...
  IMAGE_MAX_DIMENSIONS: (max: number): string => `La imagen es demasiado grande (máximo ${max}x${max} píxeles).`,
  ...
  // CRUD genéricos
  CREATE_FAILED: (entity: string): string => `No se pudo crear ${entity}.`,
  UPDATE_FAILED: (entity: string): string => `No se pudo actualizar ${entity}.`,
  DELETE_FAILED: (entity: string): string => `No se pudo eliminar ${entity}.`,
  LOAD_FAILED: (entity: string): string => `Error al cargar ${entity}.`,

  // Cantidades y stock
  QUANTITY_MUST_BE_POSITIVE: 'La cantidad debe ser un número mayor a 0.',
  STOCK_INSUFFICIENT: (available: number, inCart?: number): string =>
    inCart
      ? `Stock insuficiente. Disponible: ${available} (${inCart} ya en carrito)`
  ...
  STOCK_CHANGED: (items: StockChangedItem[]): string =>
    `Stock insuficiente para: ${items.map(i => `${i.product_name} (disponible: ${i.available}, solicitado: ${i.requested})`).join(', ')}. Otro vendedor pudo haber realizado una venta.`,
  TRANSACTION_FAILED: (type: string): string => `Error al registrar la ${type}.`,
  ...
  PDF_GENERATION_FAILED: (error?: string): string => `Error al generar PDF: ${error || 'Error desconocido'}`,
};
...
export const SUCCESS = {
  // CRUD genéricos
  CREATED: (entity: string): string => `${entity} creado correctamente.`,
  UPDATED: (entity: string): string => `${entity} actualizado correctamente.`,
  DELETED: (entity: string): string => `${entity} eliminado correctamente.`,
  ...
  // Transacciones
  TRANSACTION_CREATED: (type: string, count: number): string =>
    `${type} registrada correctamente. ${count} producto(s) procesado(s).`,
...
export const CONFIRM = {
  DELETE: (entity: string, name: string): string =>
    `¿Estás seguro de que deseas eliminar ${entity} "${name}"? Esta acción no se puede deshacer.`,
