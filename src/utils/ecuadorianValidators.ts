 ...
 */
export function validateEcuadorianCedula(cedula: string): void {
  if (!cedula) {
 ...
 */
export function validateEcuadorianRUC(ruc: string): void {
  if (!ruc) {
  ...
  if (tercerDigito < 6) {
    try {
      ...
      throw new Error(`El RUC de persona natural contiene una cédula inválida: ${(e as Error).message}`);
    }
