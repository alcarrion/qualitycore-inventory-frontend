// hooks/useTransactionActions.js
 ...
 */
export function useTransactionActions({ loadTransactions }) {
  ...
  const [formData, setFormData] = useState({ quantity: "", product: "", customer: "" });
  ...
  // --- Modal open/close ---
  const openModal = useCallback((transactionType) => {
  ...
  const handleAddToCart = useCallback(() => {
    ...
    const success = cartAddToCart(selectedProduct, formData.quantity, modal.type, showError);
    if (success) {
  ...
  const handleUpdateCartQuantity = useCallback((productId, newQuantity) => {
    ...
    cartUpdateQuantity(productId, newQuantity, modal.type, showError);
